...
class Client extends EventEmitter {
	...
	_openConnection() {
		const url = `wss://${this.options.connection.host}`;
	...
	handleMessage(chat) {
		...
		switch (chat.event) {
			...
			case "App\\Events\\ChatMessageSentEvent":
				...
				const message = chat.data.message.message;
				const self = chat.data.user.id == this.user_id ? true : false;
				this.log(`Got chat message: ${message}`);
				this.emit("chat", [channel, user, message, self]);
				break;
